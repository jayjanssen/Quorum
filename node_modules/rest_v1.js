...
var RestV1 = function() {
  ...
  that.handle = function( req, response ) {
    ...
    if( req.is_health_check ) {
      ...
      if( req.method === 'GET' || req.method === 'HEAD' ) {
        ...
        response( 501, 'Not implemented ' + req.method );
      } 
  ...
  var get_content_type = function( accept_header ) {
    ...
    // xml commented out now, because I don't understand xslt yet
    // if( /application\/xml/.test( accept_header )) {
    //   return 'application/xml';
    // } else 
    if( /text\/html/.test( accept_header )) {
      return 'text/html';
  ...
  var get_lease_response = function( req, lease, callback ) {    
    ...
    lease.fetch( function( obj ) {
      ...
      // Explicitly tell the client if they are the owner or not
      if( headers['X-Quorum-Client-ID'] === req.client_id ) {
        headers['X-Quorum-Client-Is-You'] = 'Yes';
      } else {
        headers['X-Quorum-Client-Is-You'] = 'No';
      }
      ...
      if( type === 'application/xml') {
        ...
        output = "<html><head><meta http-equiv='refresh' content='30'>";
        output += "<title>" + lease.key() + "</title>";
        output += "</head><body>";
        var lease_list_path = req.lease_prefix + "/list";
        output += "<a href='" + lease_list_path + "'>" + lease_list_path + "</a>";
        output += lease.toHTML(); 
        output += pretty_html_served_by();
        output += "</body></html>";
      } else if( type === 'application/json' ) {
        output = JSON.stringify( lease.toJSON() );
  ...
  var get = function( req, response ) {
    ...
    lease.is_valid( function( bool ) {
      ...
      get_lease_response( req, lease, function( headers, content ) {
        ...
        if( req.method === 'HEAD' ) {
          response( code, null, headers, null );
        } else {
          response( code, null, headers, content );          
        }
      });
    });        
  };
  ...
  var post = function( req, response ) {
    ...
    async.parallel({
      ...
      if( err ) { // must be an httperror
        get_lease_response( req, lease, function( headers, content ) {
          response( err.code, err.message, headers );
        });    
      } else if( results.is_owner ) {
        get_lease_response( req, lease, function( headers, content ) {
          response( 405, 'You Already Own the Lease, use PUT to renew', headers );
        });
      } else if( results.is_valid ) {
        get_lease_response( req, lease, function( headers, content ) {
          response( 409, 'Version Conflict', headers );
        });
      } else {
        // Proceed
        ...
        lease.acquire( req.client_id, req.ttl, results.content, results.version, function( results ) {
          if( results === undefined ) {
            ...
            get_lease_response( req, lease, function( headers, content ) {
              response( 201, 'Lease Acquired', headers );
            });
          }
        });
      }
  ...
  var put = function( req, response ) {
    ...
    async.parallel({
      ...
      if( err ) { // must be an httperror
        ...
        get_lease_response( req, lease, function( headers, content ) {
          response( 403, 'You do not own the lease', headers );
        });
      } else {
        // Proceed
        ...
        lease.renew( req.storage_version, results.content, function( results ) {
          if( results === undefined ) {
            ...
            get_lease_response( req, lease, function( headers, content ) {
              response( 202, 'Lease Renewed', headers );
            });
          }
        });
      }
  ...
  var delete_ = function( req, response ) {
    ...
    async.parallel({
      ...
      if( err ) { // must be an httperror
        ...
        get_lease_response( req, lease, function( headers, content ) {
          response( 404, 'Lease Free', headers );
        });
      } else {
        // Proceed
        lease.release( req.storage_version, function( results ) {
          if( results === undefined ) {
            ...
            get_lease_response( req, lease, function( headers, content ) {
              response( 204, 'Lease Released', headers );
            });
          }
        });
      }
  ...
  var get_list = function( req, response ) {        
    ...
    qs.get_all_children( req.lease_prefix, function( children ) {      
      ...
      async.map( children, function( child, cb ) {
        ...
        if( type === 'text/plain' ) {
          response( 200, null, { 'Content-Type': type }, clean_results.join( '\n' ));
        } else if( type === 'text/html' ) {
          var lease_list_path = req.lease_prefix + "/list";

          var output = "<html><head><meta http-equiv='refresh' content='30'>";
          output += "<title>" + lease_list_path + "</title>";
          output += "</head><body>"
          output += "<h2>" + lease_list_path + "</h2>";

          output += "<table>";
          output += "<tr><th>Lease</th><th>Valid</th><th>Owner</th></tr>"
          
          output += clean_results.reduce( function( str, lease ) {
            return str + lease.toHTMLRow();
          }, '' );
          ...
          response( 200, null, { 'Content-Type': type }, output );
        } else {
          response( 200, null, { 'Content-Type': 'application/json' }, JSON.stringify( clean_results ));                    
        }
  ...
  var pretty_html_served_by = function() {
    return "<h5>Served by " + os.hostname() + "</h5>";
  }
