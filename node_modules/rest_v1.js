...
var RestV1 = function() {
  ...
  var get_response = function( req, lease, callback ) {    
    async.parallel({
      'headers' : function( cb ) {
        lease.getHTTPHeaders( function( val ) { cb( null, val ); });
      },
      'content' : function( cb ) {
        //, doesn't determine "best" type, just the first one it finds
        if( /text\/html/.test( req.headers.accept )) {
          // outputs pretty HTML that shows full status of the lease
          lease.getHTML( function( obj ) { cb( null, obj ); });
        } else if( /application\/json/.test( req.headers.accept )) {
          // outputs json representation of the lease
          lease.getJSON( function( obj ) { cb( null, obj ); });
        } else if( /text\/plain/.test( req.headers.accept )) {
          // outputs a pretty plaintext version of the lease
          lease.getTXT( function( obj ) { cb( null, obj ); });
        } else {
          // default: output only the client object, meta info in headers
          lease.get_client_object( function( obj ) { cb( null, obj ); });
        }
        
      }
    }, function( err, results ) {
      log.trace( "Headers: " + util.inspect( results.headers ));
      log.trace( "Content: " + util.inspect( results.content ));
      
      callback( results.headers, results.content );
    });    
  };
  ...
  var get = function( req, response ) {
    ...
    lease.is_valid( function( bool ) {
      ...
      get_response( req, lease, function( headers, content ) {
        if( content === '' ) { //???
  ...
  var get_list = function( req, response ) {
    ...
    qs.get_all_children( req.lease_prefix, function( children ) {      
      ...
      async.map( children, function( child, cb ) {
        ...
        // response( 200, {}, util.inspect( results )); 

        // results should be the lease objs (of the paths that are leases)
        var output_obj = results.reduce( function( result, lease ) {
          if( lease !== null ) {
            result[lease.key()] = lease;            
          }
          return result;
        }, {});
        log.trace( util.inspect( output_obj ));

        response( 200, {}, JSON.stringify( output_obj ));          
      });
    });
  };
